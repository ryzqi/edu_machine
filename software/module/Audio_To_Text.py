#!/usr/bin/python3
# -*- coding:utf-8 -*-
from fileupload.seve_file import SeveFile
import requests
import datetime
import hashlib
import base64
...
def extract_and_concatenate_w(data):
    result = ""
...
class get_result(object):
    ...
    def get_fileurl(self, file_path):
        # 文件上传逻辑，现在接收file_path作为参数
        api = SeveFile(app_id=self.APPID, api_key=self.UserName, api_secret=self.Secret, upload_file_path=file_path)
    ...
    def get_result(self, file_path):
        ...
        while True:
            ...
            if isinstance(result, dict) and result['data']['task_status'] != '1' and result['data'][
                ...
                print(type(result))
                return extract_and_concatenate_w(result)
                break
...
appid = "5f261727"
apikey = "0543126960f37a2acffa6584221f0e66"
apisecret = "YWVmNTcyNTJmY2VhZjYzYjlmYzYxMWMw"

file_path = "audio/audio.pcm"
gClass = get_result(appid, apikey, apisecret)
# fileurl = gClass.get_fileurl(file_path)


def audio_to_text(audio_path):

    data0 = gClass.get_result(audio_path)

    return data0


# print(audio_to_text(file_path))
